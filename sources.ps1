Function Get-UserSelectionForArray {
    [CmdletBinding()]
    param (
    ...
    While ($true) {
        Get-Input 
        Try {
            [int]$k = Read-Host -Prompt $HelpMessage
            if (-not (($k-1) -in 0..($Array.Length -1))) {
                throw 
            }
            Break;
        } 
...
#Courtesy of an old Technet article. Unfortunely the orginal article is lost to time and I can't credit the original author
